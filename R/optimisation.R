...
ui <- fluidPage(
   ...
   sidebarLayout(
      sidebarPanel(width = 4,
         selectInput("target", "Function:",
                     c("Simple (1D)" = "simple.1d",
                       "Simple (2D)" = "simple.2d",
...
get.fn <- function(target) {
  if (target == "simple.1d") {
    ...
    g.fn <- function(x, f) rbind(2*x[1,] + 1.5 * x[2,],
                                 2*x[2,] + 1.5 *x[1,])*100
    h.fn <- function(x, f) rbind(c(2, +1.5),
                                 c(+1.5, 2))*100
    xlim <- c(-2.25, 2.25)
    ylim <- c(-1.5, 1.5)
    ...
    g.fn <- function(x, f) rbind(-400*x[1,]*(x[2,]-x[1,]^2) - 2*(1-x[1,]),
                                 200*(x[2,]-x[1,]^2))
    h.fn <- function(x, f) rbind(c(1200*x[1,]^2 - 400*x[2,] + 2, -400*x[1,]),
                                 c(-400*x[1,], 200))
    xlim <- c(-1.5, 1.5)
    ylim <- c(-0.5, 1.5)
...
if (nrow(x.default) == 1) {
    if (is.null(g.fn)) {
      g.fn <- function(x, f) { (f(x+1e-6) - f(x-1e-6)) / 2e-6 }
    }
    if (is.null(h.fn)) {
      h.fn <- function(x, f) { (f(x+1e-4) - 2*f(x) + f(x-1e-4)) / 1e-8 }
    }
  } else {
    if (is.null(g.fn)) {
      g.fn <- function(x, f) {
    ...
    if (is.null(h.fn)) {
      h.fn <- function(x, f) {
        ...
        rbind(cbind(f(x+e1) - 2 * f(x) + f(x-e1), f12),
              cbind(f12, f(x+e2)- 2 * f(x) + f(x-e2))) / 1e-8
      }
    }
  }
  list(f = f.fn, g = g.fn, h = h.fn, xlim = xlim, ylim = ylim, x.default = x.default,
       n.contour=n.contour)
}
...
take.a.step <- function(state, fn, method) {
  if (method == "simplex") {
    ...
    if (is.null(state$meta)) {
      ...
      if (dims == 1) {
        state$meta <- list(simplex=cbind(state$x, state$x + 0.1),
                           f=numeric(2),
                           size=Inf)
        state$meta$f <- c(fn$f(state$meta$simplex[,1,drop=FALSE]),
                          fn$f(state$meta$simplex[,2,drop=FALSE]))
        state$n$f <- state$n$f + 1
      } else {
        state$meta <- list(simplex=cbind(state$x, state$x + c(0.1, 0), state$x + c(0, 0.1)),
                           f=numeric(3),
                           size=Inf)
        state$meta$f <- c(fn$f(state$meta$simplex[,1,drop=FALSE]),
                          fn$f(state$meta$simplex[,2,drop=FALSE]),
    ...
    if (dims == 1) {
      ...
      state$meta$size <- max(abs(diff(range(state$meta$simplex[1,]))),
    ...
    if (is.null(state$Hinv)) {
      ...
      state$Hinv <- EH$vectors %*% diag(1/pmax(abs(EH$values), 1e-6),
                                        nrow=nrow(state$x),
                                        ncol=nrow(state$x)) %*% t(EH$vectors)
    }
    ## A proper implementation would already have the gradient, so don't count it
    g0 <- fn$g(state$x, fn$f)
    if (state$n$g == 0) {
...
server <- function(input, output, session) {
  state <- reactiveValues(x.state = list(x = get.fn("simple.1d")$x.default,
                                         converged=FALSE,
                                         n=list(f=1, g=0, h=0)),
  ...
  output$info <- renderText({
    ...
    if (input$method == "simplex") {
      if (is.null(state$x.state$meta)) {
        convergence.str <- paste0("|simplex| = ",
                                  signif(Inf, 6))
      } else {
        convergence.str <- paste0("|simplex| = ",
  ...
  output$targetPlot <- renderPlot({
    ...
    if (nrow(state$fn$x.default) == 1) {
      ...
      contour(xx, yy, matrix(state$fn$f(t(xygrid)), length(xx), length(yy)), n=state$fn$n.contour,
              xlim=xlim, ylim=ylim,
    ...
    if (ncol(state$x.history) > 0) {
      if (nrow(state$x.history) == 1) {
        fval <- vapply(seq_len(ncol(state$x.history)),
                       function(idx) state$fn$f(state$x.history[1, idx]),
                       1.0)
        points(state$x.history[1,], fval, pch=19, col=6)
        if (input$method == "simplex") {
          if (!is.null(state$x.state$meta)) {
            lines(state$x.state$meta$simplex[1,,drop=TRUE], state$x.state$meta$f, col=2)
          }
        } else if (input$method == "gradient") {
          curve(state$fn$f(state$x.state$x) + (x-state$x.state$x)*state$fn$g(state$x.state$x, state$fn$f),
                add=TRUE, col=2)
        } else if (input$method == "newton") {
          H <- state$fn$h(state$x.state$x, state$fn$f)
          EH <- eigen(H)
          H2 <- EH$vectors %*% diag(pmax(abs(EH$values), 1e-6),
                                    nrow=nrow(state$x.state$x),
                                    ncol=nrow(state$x.state$x)) %*% t(EH$vectors)
          curve(state$fn$f(state$x.state$x) + (x-state$x.state$x)*state$fn$g(state$x.state$x, state$fn$f) +
                  (x-state$x.state$x)^2/2*H, add=TRUE, col=2)
          curve(state$fn$f(state$x.state$x) + (x-state$x.state$x)*state$fn$g(state$x.state$x, state$fn$f) +
                  (x-state$x.state$x)^2/2*H2, add=TRUE, col=4)
        ...
        if (input$method == "simplex") {
          if (!is.null(state$x.state$meta)) {
            lapply(state$x.state$meta$simplex.history,
                   function(x) lines(t(x[, c(1, 2, 3, 1), drop=FALSE]), col=4))
            lines(t(state$x.state$meta$simplex[,c(1, 2, 3, 1),drop=FALSE]), col=2)
          }
        } else if (input$method == "gradient") {
            gg <- state$fn$g(state$x.state$x, state$fn$f)
            gg <- -gg/sqrt(sum(gg^2)) * ifelse(is.null(state$x.state$meta), 0.1, state$x.state$meta$step.length)
            lines(t(cbind(state$x.state$x, state$x.state$x + gg)), col=2)
          } else if (input$method == "newton") {
          f0 <- state$fn$f(state$x.state$x)
          gg <- state$fn$g(state$x.state$x, state$fn$f)
          H <- state$fn$h(state$x.state$x, state$fn$f)
          EH <- eigen(H)
          H2 <- EH$vectors %*% diag(pmax(abs(EH$values), 1e-6),
                                    nrow=nrow(state$x.state$x),
                                    ncol=nrow(state$x.state$x)) %*% t(EH$vectors)
          ad.quad(f0, g = gg, H = H, x = state$x.state$x, state$fn$xlim, state$fn$ylim, col=2)
          ad.quad(f0, g = gg, H = H2, x = state$x.state$x, state$fn$xlim, state$fn$ylim, col=4)
        } else if (input$method == "bfgs") {
          f0 <- state$fn$f(state$x.state$x)
          gg <- state$fn$g(state$x.state$x, state$fn$f)
          H <- state$fn$h(state$x.state$x, state$fn$f)
